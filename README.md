...
- **robots.txt**: `static/robots.txt` no longer disallows `/post/`, `/tags/`, `/page/` — the per-page `noindex` from `head.html` covers those, and removing the disallow lets Google crawl pagination links to discover deeper-archive posts. `/cv/` stays disallowed (only the rendered HTML/PDF files are allowed).
